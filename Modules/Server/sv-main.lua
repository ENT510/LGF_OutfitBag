...
lib.callback.register("LGF_OutfitBagOx.SyncRemoveBag", function(source, data)
    ...
    if Bags[data.bagID] then
        local success, resp = exports.ox_inventory:AddItem(source, data.ItemName, 1, {
            bagID = data.bagID,
            description = locale('bag_description', data.bagID)
        })
...
lib.callback.register("LGF_OutfitBagOx.saveCurrentOutfit", function(source, Skin, bagID, OutfitName, Slot)
    ...
    if result then
        Shared.notify(source, {
            title = locale('outfit_saved_title'),
            description = locale('outfit_saved_with_code', uniqueCode),
            type = 'success',
        })
        return uniqueCode
    else
...
lib.callback.register("LGF_OutfitBagOx.addOutfitByCode", function(source, data)
    ...
    if not result or #result == 0 then
        Shared.notify(source, {
            title = locale('outfit_not_found_title'),
            description = locale('outfit_code_invalid', data.codeName),
            type = 'error',
        })
        return nil
    end
    ...
    if insertResult then
        Shared.notify(source, {
            title = locale('outfit_added_title'),
            description = locale('outfit_added_desc', outfit.outfit_code),
            type = 'success',
        })
        return {
